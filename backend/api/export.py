...
async def export_csv(
    ...
    if metric_type == "cpu":
        writer = csv.DictWriter(output, fieldnames=["timestamp", "cpu_percent"])
        writer.writeheader()
        for row in data:
            writer.writerow({
        ...
        writer = csv.DictWriter(output, fieldnames=["timestamp", "total", "available", "percent", "used"])
        writer.writeheader()
        for row in data:
            writer.writerow({
                "timestamp": row.get("timestamp"),
        ...
        writer = csv.DictWriter(output, fieldnames=["timestamp", "read_count", "write_count", "read_bytes", "write_bytes"])
        writer.writeheader()
        for row in data:
            writer.writerow({
                "timestamp": row.get("timestamp"),
        ...
        writer = csv.DictWriter(output, fieldnames=["timestamp", "bytes_sent", "bytes_recv", "pack_sent", "pack_recv"])
        writer.writeheader()
        for row in data:
            writer.writerow({
                "timestamp": row.get("timestamp"),
